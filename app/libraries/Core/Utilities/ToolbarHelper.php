...
class ToolbarHelper
{
    ...
    public static function getInstance($moduleName, $controllerName)
    {
        if(!is_object(self::$instance)) {
            self::$instance = new ToolbarHelper($moduleName, $controllerName);
        }
     ...
     * @param string $str
     */
    public function addTitle($str = 'dashboard')
    {
        $this->title[] = __($str);
    }
     ...
     */
    public function addNewButton($rule = 'new', $buttonLink = null, $buttonName = 'add_new', $buttonIconClass = 'glyphicon glyphicon-plus', $buttonTypeClass = 'btn btn-success', $onClickEvent = null)
    {
     ...
     */
    public function addDeleteButton($rule = 'delete', $buttonLink = null, $buttonName = 'delete', $buttonIconClass = 'glyphicon glyphicon-remove', $buttonTypeClass = 'btn btn-danger delete', $onClickEvent = 'return Core.deleteSubmit(this);')
    {
        if($this->_isAllowed($rule)) {
            $this->buttons[] = $this->renderButton($rule, $buttonName, $buttonLink, $buttonIconClass, $buttonTypeClass, $onClickEvent);
        }
     ...
     * @param string $onClickEvent
     */
    public function addTrashButton($rule, $buttonLink = null, $buttonName = 'trash', $buttonIconClass = 'glyphicon glyphicon-trash', $buttonTypeClass = 'btn btn-danger delete', $onClickEvent = 'return Core.trashSubmit(this);')
    {
        if($this->_isAllowed($rule)) {
            $this->buttons[] = $this->renderButton($rule, $buttonName, $buttonLink, $buttonIconClass, $buttonTypeClass, $onClickEvent);
        }
     ...
     * @param string $onClickEvent
     */
    public function addPublishedButton($rule = 'publish', $buttonLink = null, $buttonName = 'published', $buttonIconClass = 'glyphicon glyphicon-check', $buttonTypeClass = 'btn btn-success btn-green', $onClickEvent = 'return Core.publishedSubmit(this);')
    {
        if($this->_isAllowed($rule)) {
            $this->buttons[] = $this->renderButton($rule, $buttonName, $buttonLink, $buttonIconClass, $buttonTypeClass, $onClickEvent);
        }
     ...
     * @param string $onClickEvent
     */
    public function addUnPublishedButton($rule = 'unPublish', $buttonLink = null, $buttonName = 'unpublished', $buttonIconClass = 'glyphicon glyphicon-share', $buttonTypeClass = 'btn btn-warning btn-orange', $onClickEvent = 'return Core.unPublishedSubmit(this);')
    {
        if($this->_isAllowed($rule)) {
            $this->buttons[] = $this->renderButton($rule, $buttonName, $buttonLink, $buttonIconClass, $buttonTypeClass, $onClickEvent);
        }
     ...
     * @param string $onClickEvent
     */
    public function addEditButton($rule = 'edit', $buttonLink = null, $buttonName = 'edit', $buttonIconClass = 'glyphicon glyphicon-edit', $buttonTypeClass = 'btn btn-primary', $onClickEvent = 'return Core.editButtonSubmit(this);')
    {
        if($this->_isAllowed($rule)) {
            $this->buttons[] = $this->renderButton($rule, $buttonName, $buttonLink, $buttonIconClass, $buttonTypeClass, $onClickEvent);
        }
     ...
     * @param string $onClickEvent
     */
    public function addSaveButton($rule = '', $buttonLink = null, $buttonName = 'save', $buttonIconClass = 'glyphicon glyphicon-floppy-saved', $buttonTypeClass = 'btn btn-primary', $onClickEvent = 'return Core.submitForm();')
    {
        if($this->_isAllowed($rule)) {
            $this->buttons[] = $this->renderButton($rule, $buttonName, $buttonLink, $buttonIconClass, $buttonTypeClass, $onClickEvent);
        }
     ...
     * @param string $onClickEvent
     */
    public function addSaveAndEditButton($rule, $buttonLink = null, $buttonName = 'save', $buttonIconClass = 'glyphicon glyphicon-floppy-saved', $buttonTypeClass = 'btn btn-primary', $onClickEvent = "eturn Core.saveAndEditForm('save_and_edit');")
    {
        if($this->_isAllowed($rule)) {
            $this->buttons[] = $this->renderButton($rule, $buttonName, $buttonLink, $buttonIconClass, $buttonTypeClass, $onClickEvent);
        }
     ...
     * @param string $buttonTypeClass
     */
    public function addUpdateButton($rule = 'update', $buttonLink = null, $buttonName = 'update', $buttonIconClass = 'glyphicon glyphicon-cloud-download', $buttonTypeClass = 'btn btn-primary')
    {
        if($this->_isAllowed($rule)) {
     ...
     * @param string $onClickEvent
     */
    public function addCancelButton($rule, $buttonLink = null, $buttonName = 'cancel', $buttonIconClass = 'glyphicon glyphicon-ban-circle', $buttonTypeClass = 'btn btn-warning', $onClickEvent = '')
    {
        if($this->_isAllowed($rule)) {
            $this->buttons[] = $this->renderButton($rule, $buttonName, $buttonLink, $buttonIconClass, $buttonTypeClass, $onClickEvent);
        }
     ...
     */
    public function addBackButton($buttonLink = '#', $buttonName = 'back', $buttonIconClass = 'glyphicon glyphicon-circle-arrow-left', $buttonTypeClass = 'btn btn-purple', $onClickEvent = 'javascript:history.go(-1); return false;', $buttonIconClassDir = 'left')
    {
        if($buttonLink != '#') {
     ...
     * @return string
     */
    public function renderPublishedLink($rule, $item, $link, $title = [0 => "unpublished_this_item", 1 => "published_this_item"], $iconLink = "fa fa-check-circle")
    {
        if($this->_isAllowed($rule)) {
            if($item->published == 1) {
                $title = __($title[1]);
