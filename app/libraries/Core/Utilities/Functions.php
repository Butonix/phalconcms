...
use Core\Sidebar;
use Core\Translate;
use Core\Utilities\URLify;

function get_sidebar($sidebarName)
{
    return Sidebar::getSidebar($sidebarName);
}
 ...
 */
function load_frontend_router($router)
{
    // Get frontend module
    $frontendModule = get_child_folder(APP_PATH . '/modules/frontend/');
    ...
    foreach($frontendModule as $module) {
        ...
        $fileRoute = APP_PATH . "/frontend/{$module}/{$routerClass}.php";
...
function __($code, $arrayParams = null)
{
    ...
    $translate = Translate::getInstance()->getTranslate();
...
function seo_header()
{
    return SEO::getInstance()->__toString();
}
 ...
 * @return string
 */
function seo_header_prefix()
{
    return SEO::getInstance()->getHeaderPrefix();
}
 ...
 */
function load_widget_file($location = 'frontend')
{
    $allWidget = get_child_folder(APP_PATH . "/widgets/{$location}/");

    foreach($allWidget as $w) {
        $widgetPath = APP_PATH . "/widgets/{$location}/" . $w . '/' . $w . '.php';
        if(file_exists($widgetPath)) {
            ...
            $flashSession->error(__('widget_not_found_in_location', ['1' => $w, '2' => $location]));
        }
    }
}
...
function check_resource($path, $moduleBaseName, $location)
{
    $baseActions = [
        ...
        'deleteallcache' => 'delete_all_cache',
    ...
    if(file_exists($path)) {
        ...
        if(isset($resource) && is_array($resource)) {
            ...
            if(!isset($resource['class_name'])) {
                $resource['class_name'] = 'Backend\\' . $moduleBaseName . '\\Module';
            }
            ...
            if(!isset($resource['acl'])) {
                if(DEBUG) die(__('message_module_resource_must_acl', ['1' => $path]));
                return false;
            }

            if(isset($resource['acl'])) {
                if(!is_array($resource['acl'])) {
                    if(DEBUG) die(__('message_module_resource_acl_must_be_array', ['1' => $path]));
                    return false;
                }

                if(!count($resource['acl'])) {
                    if(DEBUG) die(__('message_module_resource_acl_must_be_rule', ['1' => $path]));
                    return false;
                }
            }
            ...
            foreach($resource['acl'] as $r) {
                if(!isset($r['controller'])) {
                    if(DEBUG) die(__('message_module_resource_acl_item_must_controller', ['1' => $path]));
                    return false;
                }
                ...
                if(!isset($r['rules'])) {
                    if(DEBUG) die(__('message_module_resource_acl_item_must_rules', ['1' => $path]));
                    return false;
                }
                if(!is_array($r['rules']) || !count($r['rules'])) {
                    if(DEBUG) die(__('message_module_resource_acl_item_must_rules_item', ['1' => $r['controller'], '2' => $path]));
                    return false;
                }
                foreach($r['rules'] as $ruleItem) {
                    if(!is_array($ruleItem)) {
                        if(DEBUG) die(__('message_module_resource_structure_rules_in_controller_on_section_acl_item_error', ['1' => $r['controller'], '2' => $path]));
                        return false;
                    }
                }
                ...
                foreach($r['rules'] as $rule) {
                    ...
                    if(!isset($rule['action'])) {
                        die(__('message_module_resource_must_action_in_rules_on_controller', ['1' => $r['controller'], '2' => $path]));
                    }
                    ...
                    if(isset($rule['action'])) {
                        ...
                        if(strlen($tmp['sub_action'])) {
                            ...
                            if($sub_action != $rule['sub_action']) {
                                if(DEBUG) {
                                    die(__('message_module_resource_please_check_sub_action_in_rules_on_controller', ['1' => $rule['sub_action'], '2' => $r['controller'], '3' => $path]));
                                }
                            }
                        }
                    }
