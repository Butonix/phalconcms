            ...
            foreach($files as $file) {
                if(method_exists($file, 'getKey') && $file->getKey() == $inputName && $file->getName() != null) {
                    ...
                    if(!self::checkFileType($fileType)) {
                        $return['status'] = false;
                        $return['message'] = __('upload_image_failed_because_file_type_not_a_image');
                        return $return;
                    }
                    ...
                    if($fileSize > $maxFileSize * 1024 * 1024) {
                        $return['status'] = false;
                        $return['message'] = __('upload_image_failed_because_image_size_too_large', [$maxFileSize]);
                        return $return;
                    }
                    ...
                    if($file->moveTo(ROOT_PATH . '/public/' . $folderImages . DS . $newFile)) {
                        $return['status'] = true;
                        $return['message'] = __('upload_image_successfully');
                        ...
                        $return['status'] = false;
                        $return['message'] = __('upload_image_failed');
