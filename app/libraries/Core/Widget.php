...
class Widget
{
    ...
    {
        $files = get_files_in_folder(APP_PATH . '/widgets/frontend/' . $widgetName . '/views/', 'volt', true);
        array_unshift($files, 'default');
        $overrideWidgetFolder = APP_PATH . '/templates/frontend/' . $this->config->frontendTemplate->defaultTemplate . '/widgets/' . $widgetName . DS;
        ...
        return '<p><label for="' . $this->getFieldId('_layout') . '">' . __('layout') . '</label>'
			. Tag::select([
				$this->getFieldName('_layout'),
				$selectItems,
    ...
    public final function getForm($openClass = false)
    {
        if($this->validation) {
            ...
            $result = '<div class="widget_active" data-content="' . $this->widget_class_name . '" data-content-id="' . $this->_id . '" id="p-widget-id-' . $this->_id . '"><div class="widget-title ' . $class . '">' . __($title) . '<span class="caret"></span></div><div class="form" style="' . $style . '"><form action="/" method="post" id="widget-id-' . $this->_id . '">' . $this->form() . $this->_layout . '<input type="hidden" name="widget_id" value="' . $this->_id . '"><div class="widget-control"><a href="#" data-loading-text="Saving..." data-complete-text="Save" class="btn btn-sm btn-success widget-control-save right">Save</a><a href="#" class="btn btn-sm btn-warning widget-control-delete">Delete</a></div></form></div></div>';
            return $result;
        } else {
    ...
    public final function getWidget()
    {
        ...
        $overrideFolder = APP_PATH . '/templates/frontend/' . $this->config->frontendTemplate->defaultTemplate . '/widgets/';
        ...
        if(file_exists($overrideFile)) {
            ...
            $this->view->setViewsDir(APP_PATH . '/widgets/frontend/' . $this->_widget_name . DS);
    ...
    {
        $result = '<div class="widget_active col-sm-6 widget_new widget" data-content="' . strtolower(get_class($this)) . '"><div class="widget_active_panel"><div class="widget-title">' . __($this->_title) . '</div><div class="widget-description">' . __($this->_description) . '</div></div></div>';
        return $result;
    }
