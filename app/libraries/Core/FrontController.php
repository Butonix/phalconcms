...
class FrontController extends PController
{
    /**
     * @var Users
     */
    ...
    public function initialize()
    {
        ...
        if(method_exists($this->dispatcher, 'getControllerName')) {
            ...
            die(__FILE__ . 'Phalcon CMS cannot running!');
        }

        $_action = $this->dispatcher->getActionName();

        $routerName = 'Router' . ucfirst($this->dispatcher->getModuleName()) . 'Helper';
        $routerFile = APP_PATH . '/modules/frontend/' . $this->dispatcher->getModuleName() . '/' . $routerName . '.php';
