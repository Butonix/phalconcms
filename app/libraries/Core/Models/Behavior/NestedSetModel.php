 ...
 */
class NestedSetModel extends Behavior implements BehaviorInterface
{
    /**
    ...
    {
        $sql = 'SELECT parent.* FROM ' . $this->_table . ' AS node, ' . $this->_table . ' AS parent WHERE node.lft BETWEEN parent.lft AND parent.rgt AND node.id = ' . $id;
    ...
    protected function moveBefore($brother_id)
    {
        ...
        $slqUpdateParent = 'UPDATE ' . $this->_table . ' SET parents = ' . $newParent . ', lft = ' . $newLeft . ', rgt = ' . $newRight . ' WHERE id = ' . $this->_id;
    ...
    public function listItem($parents = 0, $items = 'all', $exclude_id = null, $level = 0)
    {
        ...
        $sqlItems = 'SELECT node.* FROM ' . $this->_table . ' AS node ';

        if($items == 'all') {
            $sqlItemsLR = ' WHERE node.lft >= ' . $parentNodeInfo['lft'] . ' AND node.rgt <= ' . $parentNodeInfo['rgt'] . ' ';
        } else {
            $sqlItemsLR = ' WHERE node.lft > ' . $parentNodeInfo['lft'] . ' AND node.rgt < ' . $parentNodeInfo['rgt'] . ' ';
        }
