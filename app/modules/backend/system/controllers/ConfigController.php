...
use Backend\System\Forms\ConfigForm;
...
class ConfigController extends BackendController
{
    ...
    public function indexAction()
    {
        ...
        if(!is_writable(ROOT_PATH . '/var/backup')) {
            $this->flashSession->error('backup_folder_cannot_be_writable ');
            $error_is_writable = true;
        }

        if(!is_writable(APP_PATH . '/config')) {
            $error_is_writable = true;
            $this->flashSession->error('config_folder_cannot_be_writable ');
        }
        ...
        if($this->request->isPost()) {
            if($configForm->isValid($_POST, $config)) {
                ...
                foreach($configForm->getElements() as $element) {
                    ...
                    if($coreConfig && $config->{$name} != '') {
                        ...
                        if($coreConfig->is_crypt_value) {
                            $coreConfig->value = Crypt::getInstance()->encrypt($config->{$name});
                        } else {
                ...
                if($error != 0) {
                    ...
                    $configFileContent = file_get_contents(APP_PATH . '/config/config.php');
                    if(file_put_contents(ROOT_PATH . '/var/backup/config.backup_' . date('Y-m-d') . '_at_' . date('H-i-s') . '.php', $configFileContent)) {
                        ...
                        if(!file_put_contents(APP_PATH . '/config/config.php', $newConfigFileContent)) {
                            $this->flashSession->warning('config_folder_cannot_be_writable');
                            return false;
                        }
                    }
                }
