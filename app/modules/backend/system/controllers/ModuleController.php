...
class ModuleController extends BackendController
{
     ...
     * @var string
     */
    public $_model = 'Core\Models\CoreModules';
    ...
    public function indexAction()
    {
        ...
        Translate::getInstance()->addModuleLang(get_child_folder(APP_PATH . '/modules/frontend/'), 'frontend');
        ...
        $this->_toolbar->addCustomButton('deleteCache', 'delete_cache', null, 'fa fa-eraser', 'btn btn-danger delete');
    ...
    {
        // Get All Module
        $allModulesBackEnd = get_child_folder(APP_PATH . '/modules/backend/');
        $allModulesFrontEnd = get_child_folder(APP_PATH . '/modules/frontend/');
    ...
    protected function updateModuleInfo($arrayModule = [], $moduleLocale)
    {
        if(count($arrayModule) == 0 || $moduleLocale == '' || ($moduleLocale != 'backend' && $moduleLocale != 'frontend')) {
            ...
            foreach($arrayModule as $module) {
                $filePath = APP_PATH . '/' . $moduleLocale . '/' . $module . '/Resource.php';
                //Get new Module name
    ...
    {
        $path = APP_PATH . '/modules/frontend/' . $module . '/Router.php';
    ...
    {
        $path = APP_PATH . '/modules/backend/' . $module . '/Menu.php';
    ...
    public function publishAction($id = null, $redirect = null, $log = true)
    {
        if(class_exists($this->_model)) {
            ...
            $this->flashSession->error('message_you_are_must_set_model_in_child_controller');
        }
