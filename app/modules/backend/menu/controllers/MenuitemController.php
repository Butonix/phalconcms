...
use Core\Utilities\Images\Images;
use Backend\Menu\Forms\MenuItemForm;
...
class MenuItemController extends BackendController
{
     ...
     * @var string
     */
    public $_model = '\Core\Models\MenuItems';
        ...
        if($this->request->isPost()) {
            if($form->isValid($_POST)) {
                ...
                if(!$menuItem->save()) {
                    ...
                    $imageStatus = ImageHelper::uploadImages($this->request->getUploadedFiles(), self::MENU_ITEM_IMAGE_FOLDER_UPLOAD, 'thumbnail');
            ...
            if($this->request->isPost()) {
                if($form->isValid($_POST)) {
                    ...
                    if(!$menuItem->save()) {
                        ...
                        $imageStatus = ImageHelper::uploadImages($this->request->getUploadedFiles(), self::MENU_ITEM_IMAGE_FOLDER_UPLOAD, 'thumbnail', null, $menuItem->image);
                        if($imageStatus['status']) {
                            $menuItem->image = $imageStatus['imageUrl'];
