    ...
    <script type="text/javascript">
        $(function() {
            $(".available-widget .widget_active").draggable({
                connectToSortable: ".sidebar-content",
                helper: "clone",
                revert: "invalid"
            });

            $("{{ sortable }}").sortable({
                connectWith: ".sidebar-content"
            }).disableSelection();
            ...
            $(".sidebar-content").sortable({
                ...
                update: function(event, ui) {
                    ...
                    if($(".sidebar_" + sidebar_name + " .widget_active").eq(indexNewItem).hasClass('widget_new')) {
                        $(".sidebar_" + sidebar_name + " .sidebar-content .widget_active").eq(indexNewItem).remove();
            ...
            $(".sidebar").delegate('.widget-control-save', 'click', function() {
                var element = $(this);
            ...
            $('.sidebar').delegate('.widget-control-delete', 'click', function() {
            ...
            $('.sidebar').delegate('.form form', 'keypress', function(e) {
                if(e.which == 13) {
                    return false;
                }
                return true;
        ...
        function insertWidget(sidebar_name, html, index) {
            if($(".sidebar_" + sidebar_name + " .sidebar-content .widget_active").length > 0) {
                var obj;
                if(index == 0) {
                    obj = $(".sidebar_" + sidebar_name + " .sidebar-content .widget_active").eq(0);
                    $(html).insertBefore(obj);
                } else if(index > 0) {
                    obj = $(".sidebar_" + sidebar_name + " .sidebar-content .widget_active").eq(index - 1);
                    $(html).insertAfter(obj);
                }
            } else {
                $(".sidebar_" + sidebar_name + " .sidebar-content").append(html);
            }
        }
    </script>
{% endblock %}