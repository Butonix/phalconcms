...
namespace Backend\Slide\Controllers;

use Core\BackendController;
use Core\Pagination;
use Core\Models\SlideShows;
use Core\Models\SlideShowItems;
use Core\Utilities\ArrayHelper;
use Core\Utilities\ImageHelper;
use Backend\Slide\Forms\SlideShowItemForm;

class ManageSlideController extends BackendController
{
    ...
    public function slideAction($id)
    {
        ...
        $this->view->setVar('_page', Pagination::getPaginationModel($slides, $paginationLimit, $currentPage));
        ...
        if($this->request->isPost()) {
            ...
            if($form->isValid($_POST, $slideShowItems)) {
                ...
                if($slideShowItems->save()) {
                    $imageStatus = ImageHelper::uploadImages($this->request->getUploadedFiles(), self::SLIDE_SHOW_FOLDER_UPLOAD . '/' . $slideShow->slide_show_id . '/slide-item', 'image');
    ...
    public function editAction($slideID, $slideItemID)
    {
        ...
        if($this->request->isPost()) {
            if($form->isValid($_POST, $slideShowItems)) {
                if($slideShowItems->save()) {
                    $imageStatus = ImageHelper::uploadImages($this->request->getUploadedFiles(), self::SLIDE_SHOW_FOLDER_UPLOAD . '/' . $slideShow->slide_show_id . '/slide-item', 'image', null, $slideShowItems->image);
