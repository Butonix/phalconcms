<div class="row">
    <div class="col-xs-6">
        {% if _page.total_pages %}
            <div class="dataTables_info">{{ __("you_are_in_current_page_per_page", ["1" : _page.current, "2" : _page.total_pages]) }}</div>
        {% else %}
            <div class="dataTables_info">{{ __("you_are_in_current_page_per_page", ["1" : 0, "2" : _page.total_pages]) }}</div>
        {% endif %}
    </div>
    <div class="col-xs-6">
        ...
        <div class="dataTables_paginate paging_bootstrap">
            <ul class="pagination">
                {% if _page.current > 1 %}
                    ...
                    <li><a href="{{ _baseUri }}{{ router.getRewriteUri() }}?page={{_page.before}}">{{ __('prev') }}</a></li>
                {% endif %}
                ...
                {% if _page.current < _page.total_pages %}
                    <li><a href="{{ _baseUri }}{{ router.getRewriteUri() }}?page={{_page.next}}">{{ __('next') }}</a></li>
                    <li><a href="{{ _baseUri }}{{ router.getRewriteUri() }}?page={{_page.last}}">{{ __('last') }}</a></li>
                {% endif %}
            </ul>
        </div>
