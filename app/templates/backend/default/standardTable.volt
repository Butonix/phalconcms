        ...
        <div class="row cms-toolbar-helper">
            ...
            <div class="col-md-4">
                <div class="dataTables_filter">
                    <label>
                        <button type="reset" onclick="return Core.resetFilter();" class="btn btn-warning btn-sm"><i
                                    class="fa fa-undo"></i> {{ __('reset') }}</button>
                    </label>
                    <label>
                        <button type="submit" class="btn btn-success btn-sm"><i
                                    class="fa fa-filter"></i> {{ __('search') }}</button>
                    </label>
                </div>
            </div>
        </div>
        ...
        {% for item in _pageLayout %}
            ...
            {% if item['type'] is defined %}
                ...
                {% elseif item['type'] == 'index' %}
                    <th style="{{ styleInline }}" class="text-center col-stt">{{ __('stt') }}</th>
                    ...
                    {% if item['column'] is defined %}
                        {% if _filter['filter_order'] == sortColumnDefault %}
                            <th style="{{ styleInline }}" class="text-center col-active {{ class }} {% if item['sort'] is not defined or item['sort'] == true %}sorting sorting_{% if _filter['filter_order_dir']|upper == 'ASC' %}asc{% else %}desc{% endif %}{% endif %}">
                                <a {% if item['sort'] is not defined or item['sort'] == true %}onclick="Core.columnOrdering('{{ sortColumnDefault }}','{% if _filter['filter_order_dir']|upper == 'ASC' %}DESC{% else %}ASC{% endif %}')"{% endif %}
                                   href="#">
                                    {{ title }}
                                </a>
                            </th>
                        {% elseif item['column']|length > 0 %}
                            <th style="{{ styleInline }}" class="text-center col-active {{ class }}{% if item['sort'] is not defined or item['sort'] == true %} sorting{% endif %}">
                                <a {% if item['sort'] is not defined or item['sort'] == true %}onclick="Core.columnOrdering('{{ sortColumnDefault }}','desc')"
                                   href="#"{% endif %}>
                                    {{ title }}
                                </a>
                            </th>
                ...
                {% elseif column['type'] == 'image' %}
                    ...
                    <td class="{{ class }}">
                        <img width="{{ width }}"
                             ...
                             height="{{ height }}" alt="{{ __('image_thumb') }}"
                                {% if itemValue!='' %}
                ...
                {% elseif column['type'] == 'date' %}
                    <td class="{{ class }} text-center">
                        {% if itemValue and itemValue != "0000-00-00 00:00:00" %}
                            {{ date(__('date_format'), strtotime(itemValue)) }}
                        {% else %}
                            {{ __('na') }}
                        {% endif %}
                    </td>
                ...
                {% elseif column['type'] == 'active' %}
                    {% if(column['access'] == true) %}
                        <td class="{{ class }} text-center col-active">
                            {% if itemValue == 1 %}
                                <a title="{{ __('deactivate_this_item') }}"
                                   href="{{ _baseUri }}{{ column['link'] ~ "deactivate/" ~ itemPrefixValue }}/"><i
                                            class="fa fa-check-circle green"></i></a>
                            {% else %}
                                <a title="{{ __('activate_this_item') }}"
